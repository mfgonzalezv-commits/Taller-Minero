              ...
              {abierta === s.id && (
                <div className="mt-3 grid gap-2 sm:grid-cols-2">
                  ...
                  <div className="flex gap-2 sm:col-span-2">
                    {necesitaAprob && !s.aprobadaCentral && puedeComprar && <button disabled={pend || s.aprobacionSolicitada} className="n-btn-ghost text-xs px-3 py-1.5" onClick={() => correr(() => solicitarAprobacionCompra(s.id, monto, monto))}>{s.aprobacionSolicitada ? 'Aprobación ya solicitada' : 'Solicitar aprobación central'}</button>}
