 ...
 */
export async function ajusteStockConLotes(tx: Prisma.TransactionClient, p: { itemId: string; faenaId: string; cantidadNueva: number; usuarioId: string; observacion?: string; /** Stock que tenía el ítem al solicitar el ajuste: si ya cambió, no se aplica. Se verifica tras bloquear el ítem. */ stockEsperado?: number }) {
  ...
  if (p.stockEsperado !== undefined && Math.abs(antes - p.stockEsperado) > 0.005) throw new Error(`El stock cambió desde la solicitud (era ${p.stockEsperado}, ahora ${antes}): rechaza y solicita el ajuste de nuevo con un conteo actualizado`)
